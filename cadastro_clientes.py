...
def inserir():
    ...
    if not nome or not email or not senha:
        messagebox.showwarning("Campos vazios", "Preencha Nome, Email e Senha.")
        return
    ...
    try:
        ...
        cursor.execute(
            ...
            (nome, email, senha_hash, telefone, data_db)
        )
        conexao.commit()
...
def atualizar():
    sel = tree.selection()
    if not sel:
        messagebox.showwarning("Seleção", "Selecione um cliente para atualizar.")
        return
    ...
    try:
        ...
        if senha:  # Se o usuário digitou uma nova senha
            senha_hash = gerar_hash_senha(senha)
            cursor.execute(
                "UPDATE Clientes SET nome=%s, `e-mail`=%s, senha=%s, telefone=%s, dataNascimento=%s WHERE idClientes=%s",
                (nome, email, senha_hash, telefone, data_db, id_cliente)
            )
        else:  # Se o campo senha estiver vazio, mantém a senha atual
            cursor.execute(
                "UPDATE Clientes SET nome=%s, `e-mail`=%s, telefone=%s, dataNascimento=%s WHERE idClientes=%s",
                (nome, email, telefone, data_db, id_cliente)
            )
...
def remover():
    sel = tree.selection()
    if not sel:
        messagebox.showwarning("Seleção", "Selecione um cliente para remover.")
        return

    id_cliente = tree.item(sel)["values"][0]

    if not messagebox.askyesno("Confirmação", "Tem certeza que deseja remover este cliente?"):
        return
...
def iniciar():
    ...
    entry_telefone = tk.Entry(janela, width=20)
    entry_telefone.grid(row=2, column=1, pady=2, padx=5, sticky="w")

    tk.Label(janela, text="Senha").grid(row=3, column=0, sticky="w")
    entry_senha = tk.Entry(janela, width=20, show="*")
    entry_senha.grid(row=3, column=1, pady=2, padx=5, sticky="w")

    tk.Label(janela, text="Data de Nascimento (AAAA-MM-DD)").grid(row=4, column=0, sticky="w")
    entry_data_nascimento = tk.Entry(janela, width=20)
    entry_data_nascimento.grid(row=4, column=1, pady=2, padx=5, sticky="w")
    ...
    for c in ("ID", "Nome", "Email", "Telefone", "DataNascimento"):
